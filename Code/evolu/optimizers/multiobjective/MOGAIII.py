...
R = TypeVar("R")
"""
Module: NSGA-III (Non-dominance Sorting Genetic Algorithm III)
    ...
    def __init__(self, n_dim: int, scaling=None, n_points: int = None, n_partitions: int = None) -> None:
        ...
        if n_points is not None:
            ...
            if n_partitions is None:
                ...
                raise InvalidParameterException("Either provide number of partitions or number of points.")
            else:
...
class NSGAIII(NSGAII):
    def __init__(self,
        ...
        if self.reference_directions.shape[1] != problem.number_of_objectives:
            ...
            raise InvalidParameterException(
                f"Dimensionality of reference points ({self.reference_directions.shape[1]}) "
                f"must be equal to the number of objectives ({problem.number_of_objectives})"
            )
    ...
    def replacement(self, population: List[S], offsprings: List[S]) -> List[S]:
        ...
        * Deb, K., & Jain, H. (2014). An Evolutionary Many-Objective Optimization
          ...
          Evolutionary Computation, 18(4), 577â€?01. doi:10.1109/TEVC.2013.2281535.
        """
        ...
        ranking.compute_ranking(population + offsprings, k=self.population_size)
        fronts, non_dominated = ranking.ranked_sub_lists, ranking.get_sub_front(0)
        ...
        pop = np.concatenate(ranking.ranked_sub_lists)
