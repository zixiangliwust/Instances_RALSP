...
class InteractivePlot(Plot):
    """Interactive plot class using Plotly for web-based visualization.
    
    This class extends the Plot class to provide interactive web-based
    visualizations using Plotly. Plots can be exported as HTML files that
    can be opened in web browsers, allowing users to zoom, pan, and interact
    with solution data.
    
    Supports 2D, 3D, and parallel coordinates plots. Interactive plots are
    particularly useful for exploring Pareto fronts and examining solution
    details through hover tooltips and click interactions.
    
    Attributes:
        figure: Plotly figure object.
        layout: Plotly layout configuration.
        data: List of Plotly trace objects for the plot.
    
    Example:
        >>> plot = InteractivePlot(reference_front=true_front)
        >>> plot.plot(solutions, label="NSGA-II", file_name="pareto_front.html")
        >>> # Opens interactive HTML file in browser
    """
    ...
    def __init__(self,
        ...
        Args:
            title (str, optional): Plot title. Defaults to "Pareto front approximation".
            reference_front (List[S], optional): Reference Pareto front to display.
                Defaults to None.
            reference_point (list, optional): Reference point(s) to display.
                Defaults to None.
            axis_labels (list, optional): Labels for axes. Defaults to None.
        """
    ...
    def plot(self, front, label=None, normalize: bool = False, file_name: str = None, format: str = "HTML"):
        """Plot front of solutions interactively using Plotly.
        
        Creates an interactive plot that can be exported as HTML or static images.
        Supports 2D, 3D, and parallel coordinates visualization based on the
        number of objectives.
        
        Args:
            front: List of Solution objects to visualize.
            label (str, optional): Front name/label for the plot. Defaults to None.
            normalize (bool, optional): Whether to normalize objectives to [0, 1]
                range. Useful for parallel coordinates with many objectives.
                Defaults to False.
            file_name (str, optional): Output file path. If None, plot is created
                but not saved. Defaults to None.
            format (str, optional): Output format. Options: "HTML" for interactive
                HTML file, or image formats ("png", "pdf", etc.). Defaults to "HTML".
        
        Note:
            - HTML format creates interactive plots with zoom, pan, and hover
            - Image formats create static plots
            - Metadata from solutions is included in hover tooltips
        """
    ...
    def export_to_html(self, file_name: str) -> str:
        """
        ...
        html_string = (
            ...
            <html>
                ...
                <body>
                    """
                + self.export_to_div(file_name=None, include_plotlyjs=False)
                + """
                <script>                
        ...
        with open(file_name + ".html", "w") as outf:
            outf.write(html_string)
        return html_string

    def export_to_div(self, file_name=None, include_plotlyjs: bool = False) -> str:
        """
        Export as a `div` for embedding the graph in an HTML file.
        param file_name: Output file name (if desired, default to None).
